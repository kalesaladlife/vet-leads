exports.handler = async function(event) {
  ...
  try {
    ...
    const validPeople = people.filter(p => p.name && p.name !== 'Unknown').slice(0, count);
    ...
    const prospects = validPeople.map(p => ({
      ...
      industry: p.organization ? (p.organization.industry || 'Veterinary') : 'Veterinary',
      ...
      fit_score: 'Good fit',
      outreach_angle: `${p.name} at ${p.organization ? p.organization.name : 'their practice'} may benefit from ${service}.`,
      email_subject: `${service} for ${p.organization ? p.organization.name : 'your practice'}`,
      email_body: `Hi ${p.first_name || p.name}, I wanted to reach out about ${service} for ${p.organization ? p.organization.name : 'your practice'}. I'd love to schedule a quick call to learn more about your needs. Would you have 15 minutes this week?`,
    }));
